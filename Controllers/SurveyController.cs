 ...
 public IActionResult get_surveys()
{
    ...
    using (var connection = _db.CreateConnection())
    {
        using (var command = connection.CreateCommand())
        {
            ...
            {   
                Console.WriteLine($"Ошибка при получении списка анкет: {ex.Message}");
                return Ok();
            }
        }
    }
...
public async Task<IActionResult> add_survey_bd([FromBody] SurveyAddRequest request)
{
    try
    {
        ...
        using (var connection = _db.CreateConnection())
        {
            var questions = request.Criteria
                .Select((text, idx) => new 
                {
                    question_id = idx + 1,
                    text = text
            ...
            var questionsData = new { questions = questions };
            var questionsJson = JsonConvert.SerializeObject(questionsData);
            ...
            foreach (var omsuId in request.Organizations.Distinct())
            {
                await connection.ExecuteAsync(
                    @"UPDATE omsu 
                      SET list_surveys = 
                          CASE 
                              WHEN list_surveys IS NULL THEN @surveyId || ','
                              ELSE list_surveys || @surveyId || ','
                          END
            ...
            await LogSurveyCreation(connection, newSurveyId, request.Title);
...
private async Task LogSurveyCreation(IDbConnection connection, int surveyId, string surveyTitle)
{
    try
    {
        await connection.ExecuteAsync(
...
[ValidateAntiForgeryToken]
public IActionResult UpdateSurvey(int id, [FromBody] SurveyUpdateModel model)
{
    ...
    try
    {
        ...
        using (var connection = new NpgsqlConnection(_configuration.GetConnectionString("DefaultConnection")))
        {
            ...
            if (affectedRows == 0)
                return StatusCode(500, new { success = false, message = "Не удалось обновить анкету" });

                        Console.WriteLine("ОРГ ПЕРЕД ОБНОВЛЕНИЕМ");
             Console.WriteLine(string.Join(", ", model.Organizations));

            // Вызываем UpdateOmsuSurveyList только если есть организации
            if (model.Organizations != null && model.Organizations.Any())
            {
                UpdateOmsuSurveyList(connection, id, model.Organizations);
            }
        ...
        using (var connection = _db.CreateConnection())
        {
            ...
            await connection.ExecuteAsync(
                @"UPDATE omsu 
                  SET list_surveys = 
                      CASE 
                          WHEN list_surveys IS NULL THEN @NewId || ','
                          ELSE list_surveys || @NewId || ','
                      END
                  WHERE list_surveys LIKE '%' || @OldId || '%'",
                new
                {
                    NewId = newSurveyId,
                    OldId = id
...
public IActionResult delete_survey([FromBody] DeleteSurveyRequest request)
{
    ...
    try
    {
        using (var connection = _db.CreateConnection())
        {
            using (var command = connection.CreateCommand())
            {
                command.CommandText = "DELETE FROM public.history_answer WHERE id_survey = @id";
                command.Parameters.Add(new NpgsqlParameter("@id", request.surveyId));
                ...
                command.CommandText = @"
                    ...
                    SET list_surveys = 
                        TRIM(BOTH ',' FROM 
                            REPLACE(
                                REPLACE(
                                    REPLACE(list_surveys, ',' || @surveyIdStr || ',', ','),
                                ',' || @surveyIdStr, ''),
                            @surveyIdStr || ',', ''))
                    WHERE list_surveys LIKE '%' || @surveyIdStr || '%'";
                
                command.Parameters.Add(new NpgsqlParameter("@surveyIdStr", request.surveyId.ToString()));
                command.ExecuteNonQuery();
            }

            using (var command = connection.CreateCommand())
            {
                command.CommandText = "DELETE FROM public.surveys WHERE id_survey = @id RETURNING id_survey";
                command.Parameters.Add(new NpgsqlParameter("@id", request.surveyId));

                var deletedId = command.ExecuteScalar();
                if (deletedId == null)
                {
                    return NotFound(new { success = false, message = "Анкета не найдена" });
                }
        ...
        var surveys = GetSurveysFromDatabase();
        return Ok(new { 
            success = true, 
    ...
    {
        _logger.LogError(ex, $"Ошибка при удалении анкеты {request.surveyId}");
        return StatusCode(500, new { 
            success = false, 
...
private List<Survey> GetSurveysFromDatabase()
{
    ...
    using (var connection = _db.CreateConnection())
    {
        using (var command = connection.CreateCommand())
        {
            command.CommandText = @"SELECT s.id_survey, s.name_survey, s.questions, 
                                  s.date_create, s.date_open, s.date_close,
                                  COALESCE(
                                      (SELECT string_agg(o.name_omsu, ', ') 
                                       FROM public.omsu o 
                                       WHERE o.list_surveys LIKE '%' || s.id_survey || '%'), 
                                  'Не указано') AS name_omsu
                                  FROM public.surveys s";
...
public IActionResult update_survey(int id)
{
        ...
        using (var connection = _db.CreateConnection())
        {
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                using (var reader = command.ExecuteReader())
                {
                    ...
                    survey.id_survey = reader.GetInt32(0);
                    survey.name_survey = reader.GetString(1);
                    survey.questions = reader.GetString(2);
                    survey.date_create = reader.GetDateTime(3);
                    survey.date_open = reader.GetDateTime(4);
                    survey.date_close = reader.GetDateTime(5);
                    survey.description = reader.IsDBNull(6) ? null : reader.GetString(6);
                }
            }

            var allOmsu = new List<OmsuItem>();
            var selectedOmsu = new List<OmsuItem>();
            var selectedOmsuIds = new List<int>();
            var selectedOmsuNames = new List<string>();

            using (var command = connection.CreateCommand())
            {
                command.CommandText = "SELECT id_omsu, name_omsu FROM public.omsu WHERE block = 'f' ";
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        allOmsu.Add(new OmsuItem
                        {
            ...
            using (var command = connection.CreateCommand())
            {
                command.CommandText = "SELECT id_omsu, name_omsu FROM public.omsu WHERE list_surveys LIKE '%' || @surveyId || '%'";
                command.Parameters.Add(new NpgsqlParameter("@surveyId", id.ToString()));
                
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        selectedOmsu.Add(new OmsuItem
                        {
                            Id = reader.GetInt32(0),
                            Name = reader.GetString(1)
                        });
                        selectedOmsuIds.Add(reader.GetInt32(0));
                        selectedOmsuNames.Add(reader.GetString(1));
                    }
                }
            }
            ...
            if (!string.IsNullOrEmpty(survey.questions))
            {
                try
                {
                    var surveyData = JsonConvert.DeserializeObject<SurveyQuestions>(survey.questions);
                    criteria = surveyData?.questions?.Select(q => q.text).ToList() ?? new List<string>();
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Ошибка при парсинге вопросов анкеты");
                    criteria = new List<string>();
                }
            }

            ViewBag.AvailableOmsu = selectedOmsu.Select(o => o.Name).ToList();
            ViewBag.AllOmsu = allOmsu;
            ViewBag.SelectedOmsuIds = selectedOmsuIds;
            ViewBag.SelectedOmsuNames = selectedOmsuNames;
    ...
    {
        _logger.LogError(ex, $"Ошибка при получении анкеты {id} для редактирования");
        return StatusCode(500, "Произошла ошибка при загрузке анкеты");
    }
}


 private void UpdateOmsuSurveyList(NpgsqlConnection connection, int surveyId, List<int> omsuIds)
{

Console.WriteLine("ОРГАНИЗАЦИИ ДЛЯ ОБНОВЛЕНИЯ");
Console.WriteLine(string.Join(", ", omsuIds));
    try
    {
        connection.Execute(
            @"UPDATE omsu 
            SET list_surveys = 
                TRIM(BOTH ',' FROM 
                    ...
                    @surveyIdStr || ',', ''))
            WHERE list_surveys LIKE '%' || @surveyIdStr || '%'",
            new { surveyIdStr = surveyId.ToString() });
        ...
        foreach (var omsuId in omsuIds.Distinct())
        {
            connection.Execute(
                ...
                SET list_surveys = 
                    CASE 
                        WHEN list_surveys IS NULL OR list_surveys = '' THEN @surveyIdStr || ','
                        ELSE list_surveys || @surveyIdStr || ','
                    END
                WHERE id_omsu = @omsuId",
                new
                {
    ...
    {
        _logger.LogError(ex, "Ошибка при обновлении списка ОМСУ для анкеты ID: {SurveyId}", surveyId);
        throw;
    }
}
...
public IActionResult copy_survey(int id)
{
    try
    {
        using (var connection = _db.CreateConnection())
        {
            var survey = connection.QueryFirstOrDefault<Survey>(
                "SELECT id_survey, name_survey, description, date_open, date_close, questions FROM surveys WHERE id_survey = @id",
                new { id });
            ...
            var organizations = connection.Query<string>(
                "SELECT name_omsu FROM omsu WHERE list_surveys LIKE '%' || @id || '%'",
                new { id }).ToList();
            ...
            ViewBag.AvailableOmsu = organizations;
            ViewBag.Criteria = criteria;
    ...
    {
        _logger.LogError(ex, $"Ошибка при загрузке анкеты для копирования (ID: {id})");
