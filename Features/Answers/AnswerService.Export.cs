...
{
    public virtual async Task<AnswerGeneratedFileResult?> CreatePdfReportAsync(
        int surveyId,
        int organizationId,
        CancellationToken cancellationToken = default)
    {
        var survey = await GetSurveyInfoAsync(surveyId, cancellationToken);
        var answers = (await GetAnswerRecordsAsync(surveyId, organizationId, cancellationToken)).ToList();
    ...
    public virtual async Task<AnswerGeneratedFileResult?> CreateSignedArchiveAsync(
        int surveyId,
        int organizationId,
        CancellationToken cancellationToken = default)
    {
    ...
    public virtual async Task<AnswerGeneratedFileResult?> CreateSurveyReportAsync(
        int surveyId,
        int organizationId,
        string? type,
        CancellationToken cancellationToken = default)
    {
        var survey = await GetSurveyInfoAsync(surveyId, cancellationToken);
        var questions = await GetSurveyQuestionsAsync(surveyId, cancellationToken);
        if (survey == null || questions.Count == 0)
        {
            return null;
        }
        var rows = (await GetAnswerRecordsAsync(surveyId, organizationId, cancellationToken)).ToList();
        if (rows.Count == 0)
        {
            return null;
        }
