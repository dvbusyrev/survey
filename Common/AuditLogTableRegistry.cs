...
public static class AuditLogTableRegistry
{
    private static readonly IReadOnlyList<AuditLogTableDefinition> SourceDefinitions =
        ...
        [
            ...
            new("organization_survey", "organization_survey_l", ["id_organization_survey"], "Назначение анкеты",
                ...
                "survey", "id_survey"),
            new("survey_template", "survey_template_l", ["id_survey_template"], "Шаблон анкеты",
                "COALESCE(NULLIF(current_row.name_survey_template, ''), 'Шаблон ' || current_row.id_survey_template::text)",
                "current_row.id_survey_template::text", "'survey_template'::text", "current_row.id_survey_template::text",
                "survey_template", "id_survey_template", IsPrimaryInChain: true),
            new("survey_template_question", "survey_template_question_l", ["id_survey_template_question"], "Критерий шаблона",
                "COALESCE(NULLIF(current_row.question_text, ''), 'Вопрос ' || current_row.question_order::text || ' шаблона ' || current_row.id_survey_template::text)",
                "current_row.id_survey_template_question::text", "'survey_template'::text", "current_row.id_survey_template::text",
                "survey_template", "id_survey_template"),
            new("organization_survey_template", "organization_survey_template_l", ["id_organization_survey_template"], "Организация шаблона",
                "'Организация ' || current_row.id_organization::text || ' / шаблон ' || current_row.id_survey_template::text",
                "current_row.id_organization_survey_template::text", "'survey_template'::text", "current_row.id_survey_template::text",
                "survey_template", "id_survey_template"),
            ...
            new("survey_template_auto_creation_config", "survey_template_auto_creation_config_l", ["id_config", "id_survey_template"], "Связь автосоздания и шаблона",
                "'Конфигурация ' || current_row.id_config::text || ' / шаблон ' || current_row.id_survey_template::text",
                "current_row.id_config::text || ', ' || current_row.id_survey_template::text", "'auto_creation_config'::text", "current_row.id_config::text",
    ...
    private static readonly IReadOnlyDictionary<string, AuditLogGroupDefinition> Groups =
        new Dictionary<string, AuditLogGroupDefinition>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["survey_template"] = new("survey_template", "Шаблон анкеты", "Шаблон"),
