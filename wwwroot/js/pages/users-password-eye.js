(function () {
    ...
    function attachToOpeners() {
        const originalOpenAddUserModal = window.openAddUserModal;
        if (typeof originalOpenAddUserModal === 'function' && !originalOpenAddUserModal.__passwordEyeWrapped) {
            const wrapped = function (...args) {
                const result = originalOpenAddUserModal.apply(this, args);
                setTimeout(initUserModalPasswordEyes, 0);
                return result;
            };
            wrapped.__passwordEyeWrapped = true;
            window.openAddUserModal = wrapped;
        }

        const originalOpenEditUserModal = window.openEditUserModal;
        if (typeof originalOpenEditUserModal === 'function' && !originalOpenEditUserModal.__passwordEyeWrapped) {
            const wrapped = function (...args) {
                const result = originalOpenEditUserModal.apply(this, args);
                setTimeout(initUserModalPasswordEyes, 0);
                return result;
            };
            wrapped.__passwordEyeWrapped = true;
    ...
    function bootstrapUserPasswordEyes() {
        attachToOpeners();
        setTimeout(initUserModalPasswordEyes, 0);
    }

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', bootstrapUserPasswordEyes);
    } else {
        bootstrapUserPasswordEyes();
    }

    window.UserPasswordEye = {
        addPasswordEye,
        initUserModalPasswordEyes,
        bootstrap: bootstrapUserPasswordEyes
    };
})();
