...
// вызвать после открытия модалок
const _originalOpenAddUserModal = window.openAddUserModal;
if (typeof _originalOpenAddUserModal === 'function') {
    window.openAddUserModal = function (...args) {
        const result = _originalOpenAddUserModal.apply(this, args);
        setTimeout(initUserModalPasswordEyes, 0);
        return result;
    };
}

const _originalOpenEditUserModal = window.openEditUserModal;
if (typeof _originalOpenEditUserModal === 'function') {
    window.openEditUserModal = function (...args) {
        const result = _originalOpenEditUserModal.apply(this, args);
        setTimeout(initUserModalPasswordEyes, 0);
        return result;
    };
}

// на случай, если поля уже есть на странице
setTimeout(initUserModalPasswordEyes, 0);
})();
