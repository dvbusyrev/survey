...
Для текущей редакции последняя миграция —
`051_protect_administrator_and_organization_closure`. В последующих поставках ориентироваться нужно на
...
Для работы по HTTP создайте у сайта IIS привязку типа `http`, укажите нужный IP-адрес
и порт, например `8080`. Перенаправление на HTTPS и правило URL Rewrite для него не
нужны. При `TransportSecurity:RequireHttps=false` приложение не включает HSTS,
не перенаправляет запросы и разрешает передачу cookie авторизации по HTTP.

После публикации открыть `http://<IP-СЕРВЕРА>:8080` и проверить вход, сохранение
сессии после перезапуска пула, загрузку инструкции и отправку тестового письма.
При первом развёртывании HTTP-версии старые HTTPS-сессии завершаются автоматически:
приложение использует новые имена cookie, чтобы сохранённые браузером защищённые cookie
не мешали входу по HTTP.
HTTP не шифрует логины, пароли и содержимое запросов, поэтому такой режим следует
использовать только в доверенной внутренней сети или через VPN. Для возврата к HTTPS
установите `TransportSecurity:RequireHttps=true` и добавьте HTTPS-привязку IIS.
